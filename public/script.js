...
button.addEventListener("click", async () => {
  ...
  try {
    const res = await fetch("/.netlify/functions/groq", {
      ...
      body: JSON.stringify({
        prompt: "You are a childrens Joke Maker AI. Your Job is to create a joke for the word entered. If the word doesn't exist, tell this Wrong Word. The format I want the answered to be returned is in a list which is [word , meaning of word , joke] (only if the word has a correct spelling). Remember to return it in a proper list with the things inside it being a string. Here is the word : " + input.value
      })
    });
